...
日期：2026-06-11。本次 spike 使用 3 个历史轮和人工构造的 direction packet，检查 binding/free 分层能否保留已结算事实。样本全部完成渲染，具体 lint 和人工检查结果如下。该样本不足以证明一般情况下的渲染质量与单 pass 持平或更好。
...
- **3 个样本的 resolvedChanges 全部落地**，且落法符合 render protocol：turn 55 的魔力消耗被译成「从契约连接里被抽走一截魔力，像从水壶里倒掉一杯水」；turn 57 的 EX 幸运免死译成瓷器裂纹+金色粒子但人还站着。无一条被写成报告句。
...
3. 渲染器会补写 packet 之外的因果推演。turn 57 自行补出「真名报出去了，往后的对手都会冲着这把刀来」。该样本中的补写与情境一致，但其它 turn 仍可能产生错误推断，因此 packet 不应依赖模型自行补全必要后果。
