...
| **当前 cwd 的 project-local 配置/资源**：`.pi/settings.json`、`.pi/extensions`、`.pi/skills`、project packages 及 `.pi/npm` / `.pi/git` | `<cwd>/.pi/`    | **仍独立存在，不受该环境变量覆盖。** 当前 `.pi/settings.json` 声明两个 package；经 trust 后自动安装/加载 project resources，npm 物理安装在 `.pi/npm/`。                                                    |
