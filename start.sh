...
#
# 项目级 pi 包（例如 npm:pi-web-access）应声明在
# 项目根 .pi/settings.json 的 packages 数组中；pi 首次启动会自动安装到 .pi/npm/。
# 发布包保留 .pi/settings.json，不要打包 .pi/npm/。
#
# 首次启动时自动初始化 .pi/agent/：
...
# ---- 后台工作进程 sessions ----
# 引擎直接 fork 两条工作进程：run_parallel_line 的异步导演（detached `pi -p`，
# ADR 0005）与 run_showrunner_audit 的同步审计员（阻塞 `pi -p`，ADR 0007）；
# 两者 session（含 hidden-canonical 内容）持久落在 gitignored 的 .pi/agent/ 树下。
# 引擎首次 spawn 会自建目录；此处预建只为显式留痕。
mkdir -p .pi/agent/backstage-sessions .pi/agent/showrunner-sessions
