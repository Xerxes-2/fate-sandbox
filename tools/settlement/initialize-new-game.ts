...
import { initializeNewGame } from "../../engine/core/campaign/new-game-initialization.ts";
import { parseNewGameInitializationInput } from "../../engine/core/campaign/new-game-schema.ts";
...
export const initializeNewGameToolDefinition: FateToolDefinition = {
  ...
  parameters: Type.Object({
    ...
    protagonist: Type.Unknown({
      description:
        "human: actorId/internalName/renderName/publicIdentity/background/apparentAge/outfit/demeanor；servant additionally className/trueNameDisplay/trueNameStatus(hidden|suspected)。actorId 按公开角色身份生成至少两段 kebab-case（如 ayaka-sajyou / servant-saber），不得编码未公开真名；renderName 是正文固定用名，中文名优先。",
    }),
