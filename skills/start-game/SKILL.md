...
- `protagonist.actorId` 必填。按玩家当前扮演的公开角色身份生成至少两段 kebab-case，例如 `ayaka-sajyou`、`fuyuki-student`、`servant-saber`。
...
```json
{
  ...
  "presence": { "presentActorIds": ["servant-saber"] },
...
如果真名未公开，actor id 也只能依据公开身份，例如 `servant-saber`；不能通过 registry key 泄露 hidden-canonical。
