...
- 不要默认玩家是 Saber / 两仪式；不要把旧 session、本地 `prompts/user/` 印象或测试路线当成新游戏默认。
...
- `protagonist.actorId` 必填。按玩家当前扮演的公开角色身份生成至少两段 kebab-case，例如 `ayaka-sajyou`、`fuyuki-student`、`snowfield-saber`。
- actor id 必须稳定指向这个角色，不得编码“玩家/主角”地位，也不得包含尚未公开的从者真名。
- `presence.presentActorIds`、`masterActorId` 等引用必须使用同一个实际 actor id。
...
```json
{
  ...
  "presence": { "presentActorIds": ["snowfield-saber"] },
...
```json
{
  "actorId": "ryougi-shiki",
  "trueNameDisplay": "两仪式",
  "trueNameStatus": "revealed"
}
```

如果真名未公开，actor id 也只能依据公开身份，例如 `snowfield-saber`；不能通过 registry key 泄露 hidden-canonical。
...
- protagonist 是否存在？`actor.id` 是否等于初始化传入的 `protagonist.actorId`，且 `public.protagonistActorId` 是否指向它？
...
- 使用中文叙事。用户明确指定人称时写入开场 packet 的 `canonFacts`；未指定时不替用户选择，由 renderer 根据开场输入自行选择并保持一致。
- 不复述完整设定表。
- 只呈现玩家此刻能感知的信息。
- 末尾停在明确可行动的瞬间。
- 不说“设定已加载”“状态已初始化”。
